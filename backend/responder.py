...
async def generate_cross_sell_line(
    ...
    if available_keywords and store_name:
        prompt = (
            ...
            f"Pick 2-3 categories from the list above that a person shopping for "
            f"\"{user_query}\" would REALISTICALLY also buy in the same shopping session.\n"
            f"Think about what goes together naturally:\n"
            f"  - shirts → pants, trousers, footwear, jeans (clothing goes with clothing)\n"
            f"  - pizza → burgers, chicken, beverages (food goes with food)\n"
            f"  - laptops → accessories, headphones, mouse (tech goes with tech)\n"
            f"NEVER suggest unrelated categories even if they exist on the store. "
            f"A person buying shirts does NOT want phone or washing machine suggestions.\n\n"
            f"Write ONE short sentence (under 20 words) mentioning only the picked categories.\n"
            f"Format: 'There are also great [category1], [category2] and [category3] offers "
            f"on {store_name} if you need'\n"
            f"Rules:\n"
            f"- ONLY mention categories from the provided list — never invent new ones\n"
            f"- NEVER mention what the user already searched for\n"
            f"- Keep it natural and professional, like a helpful store assistant\n"
            f"- Vary phrasing each time. No em dashes. No exclamation marks\n"
        ...
        prompt = (
            ...
            f"Pick 2-3 categories from the list that someone searching for "
            f"\"{user_query}\" would realistically also need.\n"
            f"Write ONE short sentence (under 20 words) mentioning them.\n"
            f"Rules:\n"
            f"- ONLY mention categories from the provided list — never invent\n"
